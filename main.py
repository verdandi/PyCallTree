...
class FileParser:
    ...
    def parse(self, file_name, root_directory = ''):
        ...
        try:
            for number, line in enumerate(open(full_path, 'r'), start = 1):
                ...
                for tokenizer in self.__tokenizers:
                    ...
                    if token == tokens.FUNCTION_DECLARATION:
                        ...
                        new_token = self.__tokens_map.setdefault(
                            token_name, {'detail': '', 'declarations': [], 'definitions': []})
            ...
            print("WARNING: File '" + full_path + "' has encoding different from Unicode and will be skipped")
